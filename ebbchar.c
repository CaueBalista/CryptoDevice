...
static char iv[17];
module_param_string(iv,iv,17,0);

static char   keyInterno[16];
static char   ivInterno[16];
...
static int test_hash(const unsigned char *data, unsigned int datalen)
{
	...
	static void crypt_SOB(char escolha){
		...
		sg_init_one(&sg[1], output, DATA_SIZE);      	//      entrou no lugar do FILL_SG();
	 	...
	 	skcipher_request_set_crypt(req, &sg[0], &sg[1], DATA_SIZE, ivInterno); // ordem ods parametros: requisição, origem, destino, tamanho, iv;
